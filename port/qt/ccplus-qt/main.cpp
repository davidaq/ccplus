...
void printHelp() {
    ...
    qDebug() << "\t" << "-fps     :   frames per second of the output video, defaults to 15";
...
#define OPT(KEY, DEFAULT) (options.contains(#KEY) ? options[#KEY] : DEFAULT)
    QDir::setCurrent(OPT(dir, dirname(inputPath)));
    CCPlus::setAssetsPath(OPT(assets, dirname(argv[0]) + "assets").toUtf8().data());
    CCPlus::RenderTarget t;
    t.inputPath = inputPath.toUtf8().data();
    t.outputPath = OPT(o, inputPath + ".mp4").toUtf8().data();
    t.fps = OPT(fps, "15").toInt();
    if(t.fps < 1)
        t.fps = 1;
    CCPlus::renderFlag = 0;
