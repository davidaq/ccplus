...
CCPLUS_FILTER(ramp) {
    ...
    if (alpha < 0 || alpha > 1.001) {
        log(CCPlus::logERROR) << "Invalid alpha value in ramp effect";
        return;
    }
...
#define uchar unsigned char
    ...
    // TODO: make sure that it will be called before opacity
    // Since the alpha of the image will be overrided by it
    for (int i = 0; i < width; i++) 
        for (int j = 0; j < height; j++) {
