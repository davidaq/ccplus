...
std::vector<std::pair<float, float>> CCPlus::triangulate(const std::vector<std::pair<float, float>>& pnts) {
    Vector2dVector a;
    for (auto& p : pnts) {
        a.push_back(Vector2d(p.first, p.second));
    }
    Vector2dVector result;
    Triangulate::Process(a, result);
    std::vector<std::pair<float, float>> ret;
    for (int i = 0; i < result.size(); i++) {
        ret.push_back({result[i].GetX(), result[i].GetY()});
    }
    return ret;
}
