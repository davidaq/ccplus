...
TEST(FillTML, FillTML) {
    ...
    CCPlus::fillTML("test/res/testfilltml.json", retpath);
    using boost::property_tree::ptree;
    ptree t1;
    read_json("tmp/test5.result.tml", t1);

    EXPECT_EQ(10.0, t1.get<float>("compositions.MAIN.duration"));
    auto& l = (*t1.get_child("compositions.@1.layers").begin()).second;
    ...
    EXPECT_EQ(12, transP.size());
    std::advance(ite, 2);
    EXPECT_EQ(0, std::atof((*ite).second.data().c_str()));
    std::advance(ite, 1);
    EXPECT_EQ(121, std::atof((*ite).second.data().c_str()));
}
